...
<?php
           ...
           if (!isset($errMSG))
           {
                $insertion = "INSERT INTO service(nomS, descriptionS, photoS)
                            VALUES('$nom','$desc','$destination_image')";
              $statement =  $connexion->prepare($insertion); 
              $statement->execute();

              if($statement)
              {
                    header("location:../vue/Formulaire_Ajouter_Service.php?reponse=1");
              }
              else {
                  header("location:../vue/Formulaire_Ajouter_Service.php?reponse=2");
              }
           }
?>
</html>
