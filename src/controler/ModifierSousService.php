...
<?php
           ...
           if ($photo != "")
           {
                ...
                if (!isset($errMSG))
                {
                    $update = "UPDATE sousservice SET nomSS = '$nom', descriptionSS = '$desc', photoSS = '$destination_image' , idservice = '$service' WHERE idSS = $id ";
                    $statement =  $connexion->prepare($update); 
                    $statement->execute();

                    if($statement)
                    {
                    header("location:../vue/Formulaire_Ajouter_SousService.php?reponses=1");
                    }
                    else {
                       header("location:../vue/Formulaire_Ajouter_SousService.php?reponses=2");
                    }
                }
           }
           else {
                $update = "UPDATE sousservice SET nomSS = '$nom', descriptionSS = '$desc', idservice = '$service' WHERE idSS = $id ";
                $statement =  $connexion->prepare($update); 
                $statement->execute();
