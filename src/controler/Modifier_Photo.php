...
<?php
           ...
           if ($photo != "")
           {
                ...
                if (!isset($errMSG))
                {
                    $update = "UPDATE galerie SET nomG = '$nom', photoG = '$destination_image' WHERE idG = $id ";
                    $statement =  $connexion->prepare($update); 
                    $statement->execute();

                    if($statement)
                    {
                    header("location:../vue/Formulaire_Ajouter_Photo.php?reponses=1");
                    }
                    else {
                       header("location:../vue/Formulaire_Ajouter_Photo.php?reponses=2");
                    }
                }
           }
           else {
                $update = "UPDATE service SET nomG = '$nom' WHERE idG = $id ";
                $statement =  $connexion->prepare($update); 
                $statement->execute();
